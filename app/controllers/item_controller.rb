...
class ItemController < ApplicationController
	...
	def auto_complete_for_searchitem_title
	  ...
	  query = params[:searchitem][:title]
	  if query.to_i.to_s == query
	    # is find by number
  		find_options = { 
  			:conditions => [ "project_id in (?) and id = ?", project_ids, query.to_i], 
  			:order => "title ASC",
  			:limit => 10 }	    
    else
  		find_options = { 
  			:conditions => [ "project_id in (?) and LOWER(title) LIKE ?", project_ids, '%' + query.downcase + '%' ], 
  			:order => "title ASC",
  			:limit => 10 }
  	end
	...
	def associate
	  ...
	  query = params[:searchitem][:title]
    unless query.empty?
      if query.to_i.to_s == query then
  		  item_to = Item.find_by_id_and_project_id(query.to_i, project_ids)
  		else
  		  item_to = Item.find_by_title_and_project_id(query, project_ids)
		  end
		  unless item_to.nil?
